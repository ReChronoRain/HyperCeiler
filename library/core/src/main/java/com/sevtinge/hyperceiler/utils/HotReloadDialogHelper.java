...
public final class HotReloadDialogHelper {
    ...
    private static void showResultToast(@NonNull Activity activity,
        ...
        String message;
        HotReloadManager.ResultCode batchFailureCode = result.getBatchFailureCode();
        if (batchFailureCode != null) {
            String failureMessage = result.getBatchFailureMessage();
            message = activity.getString(
                R.string.settings_hot_reload_batch_service_error,
                shorten(failureMessage == null ? batchFailureCode.name() : failureMessage)
            );
    ...
    /**
     * 只在热重载已明确无法执行或失败时才回退到原有硬重启。
     *
     * <p>超时或 IN_PROGRESS 不能证明失败，不能贸然 kill；而 FAILED / UNSUPPORTED /
     * SERVICE_UNAVAILABLE / SERVICE_ERROR 已经明确没有完成安全切换，此时重启全部相关进程
     * 才不会留下旧 generation。</p>
     */
    private static void showRestartFallbackIfNeeded(@NonNull Activity activity,
        ...
        HotReloadManager.ResultCode batchFailureCode = result.getBatchFailureCode();
        if (batchFailureCode != null && requiresRestartFallback(batchFailureCode)) {
    ...
    private static boolean requiresRestartFallback(@NonNull HotReloadManager.ResultCode code) {
        return code == HotReloadManager.ResultCode.FAILED
            ...
            || code == HotReloadManager.ResultCode.SERVICE_UNAVAILABLE
