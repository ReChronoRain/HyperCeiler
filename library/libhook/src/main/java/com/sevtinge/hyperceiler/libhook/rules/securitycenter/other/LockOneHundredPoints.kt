...
object LockOneHundredPoints : BaseHook() {
    ...
    override fun init() {
        ...
        runCatching {
            XposedLog.d(TAG, lpparam.packageName, "LockOneHundredPoints method is $score")
            XposedLog.d(TAG, lpparam.packageName, "LockOneHundredPoints 3 method is $score3")

            if ((AppsTool.getPackageVersionCode(lpparam) <= 40001000 && !isPad) || (AppsTool.getPackageVersionCode(lpparam) <= 40011000 && isPad)) {
                XposedLog.d(TAG, lpparam.packageName, "LockOneHundredPoints old method is $scoreOld")
                scoreOld.createHook {
                    returnConstant(0)
                }
            }
