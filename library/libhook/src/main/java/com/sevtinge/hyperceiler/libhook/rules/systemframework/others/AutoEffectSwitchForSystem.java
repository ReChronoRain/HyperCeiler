...
public class AutoEffectSwitchForSystem extends BaseHook {
        ...
        findAndHookMethod("com.android.server.audio.AudioService",
                ...
                new IMethodHook() {
                    ...
                    public void after(AfterHookParam param) {
                        ...
                        registerDebug();
                        reportEarphoneState();
                        XposedLog.d(TAG, "system", "audio system ready!!");
                    }
                }
        );
    }
    ...
    public static boolean getEarPhoneStateFinal() {
        ...
        for (AudioDeviceInfo info : outputs) {
            if (info.getType() == AudioDeviceInfo.TYPE_BLUETOOTH_A2DP || info.getType() == AudioDeviceInfo.TYPE_WIRED_HEADSET ||
                ...
                XposedLog.d(TAG, "system", "getEarPhoneState: isEarPhoneConnection: true.");
                shouldWaiting = true;
                if (mHandler != null) {
        ...
        if (shouldWaiting) {
            XposedLog.d(TAG, "system", "getEarPhoneState: isWaiting: true.");
            return true;
        }
        XposedLog.d(TAG, "system", "getEarPhoneState: isEarPhoneConnection: false.");
        return false;
    }
                ...
                new ContentObserver(new Handler(mContext.getMainLooper())) {
                    ...
                    public void onChange(boolean selfChange) {
                        ...
                        if (Settings.Global.getInt(mContext.getContentResolver(), "auto_effect_switch_restore_earphone_state", 0) == 1) {
                            ...
                            Settings.Global.putInt(mContext.getContentResolver(), "auto_effect_switch_restore_earphone_state", 0);
                            XposedLog.d(TAG, "system", "restore earphone state to false!!");
                        }
                    }
                }
        );
    ...
    class EarphoneBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action != null) {
                XposedLog.d(TAG, "onReceive: action: " + action);
                switch (action) {
