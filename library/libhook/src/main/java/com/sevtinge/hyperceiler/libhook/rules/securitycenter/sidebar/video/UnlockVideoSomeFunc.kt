    ...
    override fun init() {
        ...
        if (memc) {
            differentItems.forEach { methods ->
                XposedLog.d(TAG, lpparam.packageName, "find MeMc Method is $methods")
                hook(methods)
            }
        }
        ...
        findFrc.forEach { methods ->
            ...
            if ((resolution || enhance) && counter == 1) {
                XposedLog.d(TAG, lpparam.packageName, "find Tat Method is $findTat")
            }

            if (counter == 1 && resolution) {
                XposedLog.d(TAG, lpparam.packageName, "find SuperResolution Method is $methods")
