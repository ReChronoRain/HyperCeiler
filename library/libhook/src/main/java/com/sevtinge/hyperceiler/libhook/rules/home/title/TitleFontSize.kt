...
class TitleFontSize : HomeBaseHookNew() {
    ...
    private fun initForNewHome() {
        ...
        runCatching {
            val titleTextViewClass = findClass("com.miui.home.launcher.TitleTextView")
            titleTextViewClass.findMethod {
                findOnlyClass()
            ...
            Constructors.find(titleTextViewClass).toList().createAfterHooks {
                (it.thisObject as TextView).textSize = desktopSp
            }
        }.onFailure {
