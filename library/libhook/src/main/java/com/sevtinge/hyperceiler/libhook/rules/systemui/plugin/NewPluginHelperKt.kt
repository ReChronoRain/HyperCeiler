...
import com.sevtinge.hyperceiler.libhook.rules.systemui.plugin.systemui.NewBrightnessPct
import com.sevtinge.hyperceiler.libhook.rules.systemui.plugin.systemui.NewShowVolumePct
import com.sevtinge.hyperceiler.libhook.rules.systemui.plugin.systemui.QSColor
import com.sevtinge.hyperceiler.libhook.rules.systemui.plugin.systemui.QsTileSuperBlur
...
object NewPluginHelperKt : BaseHook() {
    ...
    private fun onPluginLoaded(factory: PluginFactory) {
        ...
        if (pluginCtx == null) {
            XposedLog.e(TAG, lpparam.packageName, "plugin context is null for $componentName")
            return
        }
        ...
        when (componentName) {
            factory.componentNames(1, "miui.systemui.volume.VolumeDialogPlugin") -> {
                XposedLog.d(TAG, lpparam.packageName, "Plugin for sysui volume loaded.")
            ...
            factory.componentNames(1, "miui.systemui.controlcenter.MiuiControlCenter") -> {
                XposedLog.d(TAG, lpparam.packageName, "Plugin for sysui qs tiles && control center loaded.")
            ...
            factory.componentNames(0, "com.miui.keyguard.shortcuts.ShortcutPluginImpl") -> {
                ...
                // XposedLog.d(TAG, lpparam.packageName, "Plugin is ${factory.mComponentName}")
    ...
    private fun loadClassLoaders(
        ...
        for ((name, loader) in loaders) {
            runCatching {
                loader(classLoader)
                XposedLog.d(TAG, lpparam.packageName, "$name is loaded success.")
            }.onFailure {
                XposedLog.e(TAG, lpparam.packageName, "[$tag] $name is fail loaded, log: ${it.stackTraceToString()}" )
            }
        }
    }
