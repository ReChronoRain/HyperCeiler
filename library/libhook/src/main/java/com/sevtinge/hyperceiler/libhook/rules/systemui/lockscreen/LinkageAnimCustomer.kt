...
class LinkageAnimCustomer : BaseHook() {
    ...
    fun initAnim() {
        ...
        val listeners = animConfig!!.getObjectField("listeners") as HashSet<Any>
        listeners.add(listener!!)
    }

    override fun init() {
        findClass("com.android.keyguard.clock.animation.ClockBaseAnimation")
            .beforeHookMethod(
                "doAnimationToAod",
                Boolean::class.java,
                Boolean::class.java,
                ...
                val fromKeyguard = param.args[2] as Boolean
                if (!fromKeyguard) return@beforeHookMethod
                ...
                if (toAod) {
                    val mWallpaperHideEase =
                        param.thisObject.getObjectField("mWallpaperHideEase")
                    mWallpaperHideEase?.callMethod("setDuration", off.toLong())
                } else {
                    initAnim()
                    ...
                    animConfig?.callMethod("setEase", showEase)
                    stateStyle?.callMethod("to", arrayOf("wallpaperBlack", 0f, animConfig))

                    param.thisObject.callMethod("doAnimationToAod", false, hasNotification)
                    param.result = null
                }
            }
    }
