...
public class SetDeviceLevel extends BaseHook {
    ...
    public static Bundle checkBundle(Context context, Bundle bundle) {
        if (context == null) {
            XposedLog.e("SetWeatherDeviceLevel", "com.miui.weather2", "Context is null!");
            return null;
        }
        ...
        for (Method method : cls.getDeclaredMethods()) {
            if (method.getName().equals("transDeviceLevel")) {
                if (method.getReturnType().equals(int.class)
                    ...
                    hookMethod(method, returnConstant(order));
                }
            }
        }
        // if (getPackageVersionCode(lpparam) < 15000000)
        //     hookAllMethods(cls, "transDeviceLevel", returnConstant(order));
        // else
