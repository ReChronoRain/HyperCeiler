...
object NewFlashLight : TileUtils() {
    ...
    override fun init() {
        super.init()
        mode = _root_ide_package_.com.sevtinge.hyperceiler.module.base.tool.HookTool.mPrefsMap.getStringAsInt("security_flash_light_switch", 0)
        setPermission(mtk)
        setPermission(torch)
        setPermission(other)
        initListen()
    ...
    fun listening(mContext: Context, param: MethodHookParam) {
        if (!isListening) {
            val contentObserver: ContentObserver =
                object : android.database.ContentObserver(Handler(mContext.mainLooper)) {
