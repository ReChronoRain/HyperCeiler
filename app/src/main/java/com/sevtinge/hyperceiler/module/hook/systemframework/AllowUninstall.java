...
public class AllowUninstall implements IXposedHookZygoteInit {
    ...
    public void initZygote(StartupParam startupParam) {
        XposedBridge.log("[HyperCeiler][I][AllowUninstall]: hook all PathClassLoader Constructors");
        pathClassLoaderHook =
            XposedBridge.hookAllConstructors(PathClassLoader.class, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) {
                    ...
                    if (path.contains("/system/framework/services.jar")) {
                        XposedBridge.log("[HyperCeiler][I][AllowUninstall]: find services.jar ClassLoader");
                        try {
                            ...
                            SecurityManagerServiceClazz = XposedHelpers.findClass(
                                ...
                                servicesClassLoader);
                            XposedBridge.log("[HyperCeiler][I][AllowUninstall]: findClass SecurityManagerService$1");
                            XposedHelpers.findAndHookMethod(SecurityManagerServiceClazz,
                                "run", new XC_MethodReplacement() {
                                    ...
                                    protected Object replaceHookedMethod(MethodHookParam unused) {
                                        XposedBridge.log("[HyperCeiler][I][AllowUninstall]: hooked checkSystemSelfProtection invoke");
                                        return null;
                                    }
                                });
                            XposedBridge.log("[HyperCeiler][I][AllowUninstall]: hook method 'SecurityManagerService$1.run()'");
                        } catch (Exception e) {
                            XposedBridge.log("[HyperCeiler][E][AllowUninstall]: AllowUninstall Exception!");
                            e.printStackTrace();
                        } finally {
                            for (Unhook hook : pathClassLoaderHook) {
                                hook.unhook();
                            }
                            XposedBridge.log("[HyperCeiler][I][AllowUninstall]: unhook all PathClassLoader Constructors");
                        }
                    }
                }
            });
