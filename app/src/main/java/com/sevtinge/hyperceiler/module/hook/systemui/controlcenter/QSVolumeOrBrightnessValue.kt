...
object QSVolumeOrBrightnessValue {
    ...
    val brightnessShowStyle by lazy {
        mPrefsMap.getStringAsInt("system_ui_control_center_qs_brightness_top_value_show_value", 0)
    }
    private val volumeShow by lazy {
        mPrefsMap.getBoolean("system_ui_control_center_qs_volume_top_value_show")
    }
    val volumeShowStyle by lazy {
        mPrefsMap.getStringAsInt("system_ui_control_center_qs_volume_top_value_show_value", 0)
    }
            ...
            XposedHelpers.findAndHookMethod(VolumeSliderController, "updateIconProgress",
                Boolean::class.java, object : XC_MethodHook() {
                    override fun afterHookedMethod(param: MethodHookParam?) {
                        ...
                        str =
                            if (volumeShowStyle == 0) ((value * 100) / max).toString() + "%" else value.toString()
        ...
        if (brightnessShow) {
            ...
            XposedHelpers.findAndHookMethod(BrightnessSliderController, "updateIconProgress", object : XC_MethodHook() {
                    override fun afterHookedMethod(param: MethodHookParam?) {
                        ...
                        val topValue = item.findViewByIdName("top_text") as TextView
                        str =
                            if (brightnessShowStyle == 0) ((value * 100) / max).toString() + "%" else value.toString()
