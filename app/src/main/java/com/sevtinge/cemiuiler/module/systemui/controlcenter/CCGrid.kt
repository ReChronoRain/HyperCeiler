...
import com.sevtinge.cemiuiler.module.aod.UnlockAlwaysOnDisplay.findAndHookMethod
import com.sevtinge.cemiuiler.module.aod.UnlockAlwaysOnDisplay.lpparam
import com.sevtinge.cemiuiler.module.aod.UnlockAlwaysOnDisplay.mResHook
import com.sevtinge.cemiuiler.module.base.BaseHook.MethodHook
...
object CCGrid {
    ...
    fun initCCGrid(pluginLoader: ClassLoader?) {
        var scaledTileWidthDim = -1f
        val cols = mPrefsMap.getInt("system_control_center_cc_columns", 4)
        val rows = mPrefsMap.getInt("system_control_center_cc_rows", 4)
        if (cols > 4) {
            mResHook.setObjectReplacement(lpparam.packageName, "dimen", "qs_control_tiles_columns", cols)
        }
        findAndHookMethod("com.android.systemui.SystemUIApplication", "onCreate",
            object : MethodHook() {
                ...
                override fun after(param: MethodHookParam) {
                    if (!isHooked) {
                        isHooked = true
                        val mContext =
                            XposedHelpers.callMethod(param.thisObject, "getApplicationContext") as Context
                        ...
                        if (cols > 4) {
                            tileWidthDim = tileWidthDim.div(density)
                            scaledTileWidthDim = tileWidthDim * 4 / cols
                            mResHook.setDensityReplacement(
                                lpparam.packageName,
                                "dimen", "qs_control_center_tile_width",
                                scaledTileWidthDim
                            )
                            mResHook.setDensityReplacement(
                                "miui.systemui.plugin", "dimen", "qs_control_center_tile_width",
                                scaledTileWidthDim
                            )
                            mResHook.setDensityReplacement(
                                lpparam.packageName,
                                "dimen", "qs_control_tile_icon_bg_size",
                                scaledTileWidthDim
                            )
                            mResHook.setDensityReplacement(
        ...
        if (cols > 4) {
            Helpers.findAndHookConstructor(
        ...
        if (rows != 4) {
            Helpers.findAndHookMethod(
                ...
                object : MethodHook() {
                    override fun after(param: MethodHookParam) {
                        val collapse =
                            XposedHelpers.getObjectField(param.thisObject, "collapse") as Boolean
                        if (collapse) {
                            val pages =
                                XposedHelpers.getObjectField(param.thisObject, "pages") as ArrayList<*>
                            ...
                            var currentRow = 2
                            val records =
                                XposedHelpers.getObjectField(param.thisObject, "records") as ArrayList<*>
                            val it2: Iterator<*> = records.iterator()
                            var i3 = 0
                            var pageNow = 0
                            val bigHeader =
                                XposedHelpers.getObjectField(param.thisObject, "header")
                            while (it2.hasNext()) {
                                ...
                                if (currentRow >= rows || !it2.hasNext()) {
                                    XposedHelpers.callMethod(
                                        pages[pageNow], "setTiles", pageTiles,
                                        if (pageNow == 0) bigHeader else null
                                    )
                                    pageTiles.clear()
                                    val totalRows =
                                        XposedHelpers.getObjectField(param.thisObject, "rows") as Int
                            ...
                            while (it3.hasNext()) {
                                val next2 = it3.next()
                                val isEmpty =
                                    XposedHelpers.callMethod(next2, "isEmpty") as Boolean
                            ...
                            val adapter =
                                XposedHelpers.getObjectField(param.thisObject, "adapter")
                            XposedHelpers.callMethod(
                                param.thisObject, "setAdapter", adapter
                            )
        ...
        if (mPrefsMap.getBoolean("system_control_center_qs_tile_label")) {
            ...
            val mQSController =
                XposedHelpers.findClassIfExists("miui.systemui.controlcenter.qs.tileview.StandardTileView", pluginLoader)
            Helpers.hookAllMethods(mQSController, "init", object : MethodHook() {
                @Throws(Throwable::class)
                override fun before(param: MethodHookParam) {
                    if (param.args.size != 1) return
                    val mLabelContainer =
                        XposedHelpers.getObjectField(param.thisObject, "labelContainer") as View
                    if (mLabelContainer != null) mLabelContainer.visibility = View.GONE
                }
            })
        }
        ...
        if (mPrefsMap.getBoolean("system_ui_control_center_rounded_rect")) {
            ...
            Helpers.findAndHookMethod(
                "miui.systemui.controlcenter.qs.tileview.ExpandableIconView", pluginLoader,
                "setCornerRadius",
                ...
                object : MethodHook() {
                    ...
                    override fun before(param: MethodHookParam) {
                        val mContext =
                            XposedHelpers.callMethod(param.thisObject, "getPluginContext") as Context
                        var radius = 18f
            ...
            Helpers.findAndHookMethod(
                ...
                object : MethodHook() {
                    override fun before(param: MethodHookParam) {
                        ...
                        Helpers.findAndHookMethod(
                            "android.content.res.Resources", pluginLoader,
                            "getDrawable",
                            Int::class.javaPrimitiveType, imgHook
                        )
                        Helpers.findAndHookMethod(
                            "android.content.res.Resources.Theme", pluginLoader,
                            "getDrawable",
                            Int::class.javaPrimitiveType, imgHook
                        )
                    }
