...
object NotificationWeatherNew : BaseHook() {
    ...
    override fun init() {
        ...
        val mControlCenterDateViewClass = loadClass("com.android.systemui.controlcenter.phone.widget.ControlCenterDateView")
        ...
        loadClass("com.android.systemui.shared.plugins.PluginManagerImpl").methodFinder().first {
           ...
           after { getClassLoader ->
               ...
               if (appInfo.packageName == "miui.systemui.plugin") {
                   loadClass("miui.systemui.controlcenter.windowview.MainPanelHeaderController", classLoader).methodFinder().first {
                       name == "addClockViews"
                   }.createHook {
                       after {
